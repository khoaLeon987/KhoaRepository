        ...
        unless count.zero?
          ...
          I18n.with_options :locale => options[:locale], :scope => [:errors, :template] do |locale|
            ...
            objects.sum do |object|
              object.errors.each do |key, value|
                ...
                if value.index("^") == 0
                  ...
                  messages << "#{object.class.human_attribute_name(key)} #{value}"
                end
              end              
            end
            messages.uniq!
            ...
            header_message = if options.include?(:header_message)
              ...
              locale.t :header, :count => messages.count, :model => options[:object_name].to_s.gsub('_', ' ')
            end
            
            error_messages = messages.map { |msg| content_tag(:li, msg) }.join.html_safe
            contents = ''
